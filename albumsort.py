def album( name, year, tracks, length ):
     return {
        "Name": name,
        "Year": year,
        "Tracks": tracks,
         "Length": length
        }

albums=[
     album( 'Orchid', '1995', '8 tracks', '1 hr 11 min'),
     album( 'Morningrise', '1996', '6 tracks', '1 hr 14 min'),
     album( 'My Arms Your Hearse', '1998', '11 tracks', '1 hr 2 min'),
     album( 'Still Life', '1999', '7 tracks', '1 hr 2 min'),
     album( 'Blackwater Park', '2000', '9 tracks', '1 hr 16 min'),
     album( 'Deliverance', '2002', '6 tracks', '1 hr 1 min' ),
     album( 'Damnation', '2003','8 tracks', '43 min'),
     album( 'Ghost Reveries', '2005', '8 tracks', '1 hr 6 min'),
     album( 'Watershed', '2008', '10 tracks', '1 hr 11 min'),
     album( 'Heritage', '2011', '10 tracks', '56 min'),
     album( 'Pale Communion', '2014', '8 tracks', '55 min'),
     album( 'Sorceress', '2016', '16 tracks', '1 hr 30 min')
     ]

listt = tuple(sorted(albums, key=lambda k: k["Name"]))

##for i in listt:
##     print ("Name:", i["Name"])
##     print ("Year:", i["Year"])
##     print ("Tracks:", i["Tracks"])
##     print ("Length:", i["Length"])
##     print ("__________________")
##print("\nNow time for release order:\n")
##for i in sorted(albums, key=lambda x: x["Year"]):
##     print ("Name:", i["Name"])
##     print ("Year:", i["Year"])
##     print ("Tracks:", i["Tracks"])
##     print ("Length:", i["Length"])
##     print ("__________________")

MEMBERS = {
	"mikael",
	"and the rest"
	}
